...
`OK (372 tests, 1599 assertions)` — ver [04 — Testes](04-testes.md) para o que
...
# 3) medir
python cdp_lcp.py "<ws://…/devtools/page/…>" "https://www.powerarena.example.com/"
...
## Achado 3 — `http://powerarena.example.com` cai no wp-login

```
http://powerarena.example.com/
  → 301 → https://www.powerarena.example.com/wp-admin/
  → 302 → https://www.powerarena.example.com/wp-login.php?redirect_to=…%2Fwp-admin%2F
```
...
> site.** Uma regra `powerarena.example.com/ → https://www.powerarena.example.com` no
> painel da Hostinger causa **loop infinito em todos os caminhos**, porque
> `www.powerarena.example.com` é **alias de DNS** do domínio sem `www` — a regra casa
...
> `https://powerarena.example.com/` → 301 → `https://www.powerarena.example.com/`).
>
...
## Produção — Arena 0.2.9 (2026-08-01)

Publicação incremental executada sem alterar GTM, WPBakery ou a configuração do
LiteSpeed. Backup anterior ao deploy salvo no servidor em
`wp-content/.codex-backups/powerarenaarena-20260801-0.2.8-before-0.2.9/`, protegido
por `.htaccess`, com tema completo, metadados e arquivos das imagens afetadas.

### Mudanças verificadas no HTML/rede

| Item | Antes | Depois |
|---|---:|---:|
| banner mobile | 1170×351 · 121,6 KiB | 640×192 · **32,3 KiB** |
| fontes baixadas | 8 · 153,4 KiB | 4 · **86,7 KiB** |
| imagens (run representativo) | 260,9 KiB | **171,6 KiB** |
| scripts CF7 na home | ~13 KiB | **0** |
| SDK OneSignal no parser/caminho do LCP | ~43 KiB | **0**; interação ou `load` + 4 s |
| payload mobile | 1.101 KiB | **730–890 KiB** |

O `vc_single_image` permanece sendo o componente do WPBakery e continua
editável. O tema apenas devolve o `srcset` que o componente descartava. Foram
regeneradas somente as variantes ausentes do banner e dos cards atuais da home;
não houve regeneração em massa da biblioteca.

### Lighthouse após o deploy

Três execuções na URL canônica, host do antivírus bloqueado:

| Perfil | Score | LCP | FCP | TBT | CLS | Payload |
|---|---:|---:|---:|---:|---:|---:|
| mobile — melhor execução | 93 | 2,8 s | 2,2 s | 10 ms | 0,000 | 730 KiB |
| mobile — mediana | 90 | 3,3 s | 2,2 s | 10 ms | 0,000 | 890 KiB |
| desktop — mediana | **98** | **1,1 s** | 0,6 s | 0 ms | 0,000 | 1.103 KiB |

A variação mobile não veio das mudanças do tema: nas duas execuções lentas uma
segunda cópia de `gtag.js` (159,6 KiB) entrou na rede; na execução 93 ela não
entrou. A consolidação GTM/GA4 foi explicitamente adiada, portanto a mediana deve
ser lida com essa variável de negócio ainda aberta. Mesmo com ela, o payload
mediano caiu 211 KiB e o melhor resultado preservou 93/LCP 2,8 s.

### Servidor

- OPcache ativo (`opcache.memory_consumption=2048`).
- Drop-in de object cache do LiteSpeed presente; nenhum plugin de cache foi
  instalado ou substituído.
- O mobile continua com resposta de documento entre ~480 e 740 ms e segue sendo
  a principal oportunidade de timing.
- A conta SSH não disponibiliza `/var/spool/cron`; o cron real precisa ser
  criado pelo hPanel. **Não** definir `DISABLE_WP_CRON` antes de confirmar esse
  agendamento, pois há filas recorrentes a cada minuto.

### Smoke test pós-publicação

Home, matéria, categoria, categoria página 2 e busca retornaram HTTP 200; a URL
de prova inexistente retornou 404. Tema ativo confirmado em 0.2.9 e caches
Autoptimize/LiteSpeed purgados.

## Produção — Arena 0.2.10/0.2.11 (2026-08-01)

Rodada iniciada a partir do relatório compartilhado do PageSpeed (83, LCP 3,9 s).
O relatório escolheu o **segundo** tile grande da home como LCP, mas só o primeiro
recebia `fetchpriority="high"`. Os dois candidatos grandes passaram a receber
prioridade; o terceiro tile visível continua apenas `eager`, sem promover todas
as imagens da página.

Também foi registrado `arena-card-small` (240×135) e foram geradas 99 variantes
recentes (um anexo antigo já estava ausente). `sizes` agora distingue card
compacto, blog e arquivo. Na home atual, parte dos compactos repete as mesmas
imagens do hero: o navegador reutiliza a cópia grande já em cache em vez de fazer
um segundo download pequeno — comportamento correto, embora o diagnóstico por
elemento do Lighthouse continue sugerindo redimensionamento.

### A/B de CSS do WPBakery

O `js_composer_optimized.min.css` foi carregado de forma assíncrona numa cópia
dinâmica da produção, sem expor visitantes. Cinco execuções por variante:

| Variante | Score mediano | FCP | LCP | CLS máximo |
|---|---:|---:|---:|---:|
| controle | 91 | 2,15 s | 3,20 s | 0,000 |
| CSS-base assíncrono | 91 | 2,15 s | 3,20 s | 0,000 |
| prioridade dupla | 91 | 2,15 s | 3,21 s | 0,000 |
| combinado | 91 | 2,15 s | 3,22 s | 0,000 |

Sem ganho de mediana, o CSS assíncrono **não foi ativado**. WPBakery e a divisão
atual Autoptimize/LiteSpeed foram preservados.

### Matérias e reCAPTCHA do wpDiscuz

Nas matérias, a imagem destacada já estava correta (`eager`, `skip-lazy`,
`fetchpriority="high"`). A oportunidade real era o reCAPTCHA: API e dependências
somavam ~782 KiB no carregamento de toda leitura. Desde 0.2.11 ele carrega quando
`#wpdcom` chega a 600 px da viewport ou recebe foco/toque/clique.

Teste funcional em Chrome: **0 requisições** antes do scroll; depois de aproximar
os comentários, API Google e recursos gstatic carregaram normalmente.

### Lighthouse final em produção

Medianas de três execuções mobile, após purge de LiteSpeed e Autoptimize:

| Página | Score | FCP | LCP | TBT | CLS | Payload |
|---|---:|---:|---:|---:|---:|---:|
| home | **93** | 2,16 s | 2,95 s | 3 ms | 0,000 | 619 KiB |
| matéria Razer | **93** | 2,15 s | 2,83 s | 2 ms | 0,000 | 637 KiB |
| matéria Valve | **93** | 2,17 s | 2,86 s | 2 ms | 0,000 | 631 KiB |

Todas as nove execuções tiveram um `gtag.js`, um `page_view` e zero GTM. O
reCAPTCHA não entrou no carregamento inicial das seis execuções de matéria.
Backups: `powerarenaarena-20260801-0.2.9-before-0.2.10` e
`powerarenaarena-20260801-0.2.10-before-0.2.11`.
