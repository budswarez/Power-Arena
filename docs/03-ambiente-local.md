...
| **Node.js** | 18+ | build dos assets (Vite) e o próprio `wp-env` |
...
npm install          # ferramentas de build + wp-env
composer install     # PHPUnit e stubs (só desenvolvimento)
npm run build        # gera assets/dist/ — obrigatório, ver observação abaixo

npx wp-env start     # sobe o WordPress (primeira vez baixa as imagens: alguns minutos)
```
...
npx wp-env start             # sobe
npx wp-env stop              # para (mantém o banco)
npx wp-env destroy           # apaga tudo, inclusive o banco
...
Esperado: `OK (353 tests, 1511 assertions)` — o número cresce conforme novos
