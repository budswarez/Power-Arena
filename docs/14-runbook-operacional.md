...
**Pré-condição:** suíte verde (`OK (372 tests, 1599 assertions)`), árvore de
...
```bash
curl -sI https://www.powerarena.example.com/                       # 200
curl -s  https://www.powerarena.example.com/ | grep -c '\[bs-'     # 0 (nenhum shortcode cru)
curl -s  https://www.powerarena.example.com/ | grep -c '<h1'       # 1
curl -s  https://www.powerarena.example.com/ | grep -c 'googleapis' # 0
curl -sI https://www.powerarena.example.com/pagina-que-nao-existe/ # 404 de verdade
```
...
3. acessar `https://www.powerarena.example.com/?arena_preview=<token>`.
...
Esperado: `OK (372 tests, 1599 assertions)`.
