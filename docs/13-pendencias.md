...
curl -sD - -o /dev/null -A "Mozilla/5.0 (Linux; Android 13) Mobile" \
  https://www.powerarena.example.com/ | grep -i x-litespeed-cache
```
...
`http://powerarena.example.com/qualquer-coisa` respondia 301 para
`https://www.powerarena.example.com/wp-admin/`, que caía no `wp-login.php`.
...
RewriteRule ^ https://www.powerarena.example.com%{REQUEST_URI} [R=301,L]
</IfModule>
...
| | Antes | Depois |
|---|---|---|
| `http://powerarena.example.com/` | → `/wp-admin/` → `wp-login.php` | → `https://www.powerarena.example.com/` (**1 redirect**) |
| `http://powerarena.example.com/csgo/cs2/` | → `/wp-admin/` | → a própria matéria |
| `https://powerarena.example.com/` | correto, mas via PHP | correto, no servidor |
| custo de PHP no redirect | `X-Processing-Time: 0.302` | **`0.000`** |
| `/evento` (as duas variantes) | → `evento.powerarena.example.com` | **preservado** |
...
| Caminho | Papel |
|---|---|
| `themes/arena/arena-child/` | **canônica** — é o que o git versiona e o que o `.wp-env.json` mapeia para `themes/arena-child` |
