...
**Goal:** Entregar as páginas que hoje caem no `index.php` mínimo — **artigo (single)**, **categoria/tag/arquivo**, **busca**, **autor** e **404** — com paridade estrutural e visual em relação ao `powerarena.example.com`, incluindo **sidebar direita**, paginação, breadcrumbs do Yoast e comentários (wpDiscuz).
...
- [ ] **Step 1:** capturar a referência (recipe que funciona: `--host-resolver-rules="MAP * 127.0.0.1, EXCLUDE www.powerarena.example.com, EXCLUDE powerarena.example.com"`) para **single**, **categoria** e **busca**; capturar as nossas equivalentes; comparar lado a lado e registrar diferenças honestamente.
