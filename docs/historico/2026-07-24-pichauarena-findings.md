# Power Arena — Achados do Reconhecimento (parcial, sem SSH)

- **Data:** 2026-07-24
- **Método:** `curl` do HTML público de `https://www.powerarena.example.com/` (somente leitura, dados públicos, sem PII). SSH por senha **não** foi possível de forma não-interativa (`sshpass` ausente) — metadados via WP-CLI pendentes de acesso por chave ou execução manual pelo usuário.
...
- **4 tipos de bloco de listagem** na home:
  ...
  - + `vc_single_image` (banner "Evento Power Arena 2026" → evento.powerarena.example.com).
